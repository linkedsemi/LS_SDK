...
uint32_t ali_pid_u32 = 18519722;
uint8_t ali_mac[TRIPLE_MAC_LEN] = {0x3c,0x5d,0x2a,0x8b,0x4b,0xc5};
uint8_t ali_secret[TRIPLE_SECRET_LEN] = {0x6b,0x4d,0x9c,0x35,0x5a,0x27,0x53,0x8f,0x59,0x72,0xdb,0x94,0x44,0xa2,0x3a,0xb0};
...
static void mesh_manager_callback(enum mesh_evt_type type, union ls_sig_mesh_evt_u *evt)
{
    ...
    case MESH_ACTIVE_REGISTER_MODEL:
    {
        ...
        model_subscribe(model_env.info[0].model_lid, 0xcfff); 
        model_subscribe(model_env.info[0].model_lid, 0xc003); 
        model_subscribe(model_env.info[1].model_lid, 0xcfff); 
        model_subscribe(model_env.info[1].model_lid, 0xc003);
        model_subscribe(model_env.info[2].model_lid, 0xcfff); 
        model_subscribe(model_env.info[2].model_lid, 0xc003); 
    }
    break;
