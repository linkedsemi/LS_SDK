...
struct button_info button1,button2,button3;
...
void ls_button_timer_init(void)
{
    ...
    button3.release_count=0;
    button3.count=0;
    button3.check_count=0;
...
void light_button_init(void)
{
    ...
    io_pull_write(LIGHT_BUTTON_1,IO_PULL_UP);
    io_exti_config(LIGHT_BUTTON_1,INT_EDGE_FALLING);
    

    io_cfg_input(LIGHT_BUTTON_2);
    io_pull_write(LIGHT_BUTTON_2,IO_PULL_UP);
    io_exti_config(LIGHT_BUTTON_2,INT_EDGE_FALLING);
    

    io_cfg_input(LIGHT_BUTTON_3);
    io_pull_write(LIGHT_BUTTON_3,IO_PULL_UP);
    io_exti_config(LIGHT_BUTTON_3,INT_EDGE_FALLING);
...
void io_exti_callback(uint8_t pin,exti_edge_t edge) // override io_exti_callback
{
    ...
    case LIGHT_BUTTON_1:
    {
       ...
       app_client_model_tx_message_handler(((on_off & 0x00000001)?APP_STATE_ON:APP_STATE_OFF),MODEL0_GENERIC_ONOFF_CLI);
    ...
    { 
        button3.count++;
       if (button3_valid == false)
       {
          button3.check_count = button3.count;
          button3_valid =true;
...
static void ls_button_timer_cb(void *param)
{
   ...
   if (button3_valid == true)
   {
       if (button3.check_count != button3.count)
       {
           button3.release_count=0;
           button3.check_count = button3.count;
       }
