...
void HAL_LSCRYPT_MSP_Init(void)
{
    ...
    rv_set_int_isr(BMC_CRYPT_IRQn, HAL_LSCRYPT_IRQHandler);
    csi_vic_clear_pending_irq(BMC_CRYPT_IRQn);
    csi_vic_enable_irq(BMC_CRYPT_IRQn);
}

void HAL_LSCRYPT_MSP_DeInit(void)
{
    SYSC_CPU->PD_CPU_CLKG[1] = SYSC_CPU_CLKG_CLR_CRYPT_MASK;
    csi_vic_disable_irq(BMC_CRYPT_IRQn);
}
