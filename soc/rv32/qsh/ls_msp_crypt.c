...
void HAL_LSCRYPT_MSP_Init(void)
{
    ...
    rv_set_int_isr(CALC_CRYPT_IRQN, HAL_LSCRYPT_IRQHandler);
    csi_vic_clear_pending_irq(CALC_CRYPT_IRQN);
    csi_vic_enable_irq(CALC_CRYPT_IRQN);
}

void HAL_LSCRYPT_MSP_DeInit(void)
{
    SYSC_SEC_CPU->PD_CPU_CLKG[1] = SYSC_SEC_CPU_CLKG_CLR_CRYPT_MASK;
    csi_vic_disable_irq(CALC_CRYPT_IRQN);
}
