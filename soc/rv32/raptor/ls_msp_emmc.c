#include "ls_msp_emmc.h"
#include "ls_hal_emmc.h"
#include "raptor.h"
#include "exception_isr.h"
#include "core_rv32.h"
#include "reg_cpu1_sys.h"
#include "field_manipulate.h"
#include "reg_sysc_awo.h"
#include "reg_pipe_sys.h"
#include "ls_soc_gpio.h"
#include "platform.h"

void HAL_LSEMMC_MSP_Init(uint32_t mapbase)
{
    // 1.选择25M时钟源
    PIPE_SYS_CFG -> EMMC_CORE_CLK_CLK_DEGLITCH_SW3_REG = 0x1; //25M
    PIPE_SYS_CFG -> EMMC_CORE_CLK_CLK_DIV0_DIV2_REG = 0x0;
    PIPE_SYS_CFG -> EMMC_TM_CLK_CLK_DEGLITCH_SW3_REG = 0x2; //25M
    PIPE_SYS_CFG -> EMMC_TM_CLK_CLK_DIV0_DIV8_REG = 0x0;
    PIPE_SYS_CFG -> EMMC_CCLK_TX_CLK_CLK_DEGLITCH_SW2_REG = 0x1;
    PIPE_SYS_CFG -> EMMC_CCLK_RX_CLK_CLK_DEGLITCH_SW3_REG = 0x1;

    // 2.使能时钟门控
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_REG0, PIPE_SYS_EMMC_TX_CLK_INV_SEL, 0x1);
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG1, PIPE_SYS_EMMC_AXI_CLK_CLK_CG0_5, 0x1); 
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG3, PIPE_SYS_EMMC_AHB_CLK_CLK_CG0_1, 0x1);
    PIPE_SYS_CFG -> EMMC_CORE_CLK_CLK_CG0_REG = 0x1;
    PIPE_SYS_CFG -> EMMC_TM_CLK_CLK_CG0_REG = 0x1; 
    PIPE_SYS_CFG -> EMMC_CCLK_TX_CLK_CLK_CG0_REG = 0x1;
    PIPE_SYS_CFG -> EMMC_CCLK_RX_CLK_CLK_CG0_REG = 0x1;

    // 3.延时一段时间，关闭时钟门控
    DELAY_US(6);
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG1, PIPE_SYS_EMMC_AXI_CLK_CLK_CG0_5, 0x0); 
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG3, PIPE_SYS_EMMC_AHB_CLK_CLK_CG0_1, 0x0);
    PIPE_SYS_CFG -> EMMC_CORE_CLK_CLK_CG0_REG = 0x0;
    PIPE_SYS_CFG -> EMMC_TM_CLK_CLK_CG0_REG = 0x0; 
    PIPE_SYS_CFG -> EMMC_CCLK_TX_CLK_CLK_CG0_REG = 0x0;
    PIPE_SYS_CFG -> EMMC_CCLK_RX_CLK_CLK_CG0_REG = 0x0;

    // 4. reset 5.释放reset
    CPU1_SYS_CFG -> PIPE_SYS_CRG_AXI_CLK_TOP_RST_N_REG0_CLR = 0x20;
    CPU1_SYS_CFG -> PIPE_SYS_CRG_AXI_CLK_TOP_RST_N_REG0 = 0x20;
    CPU1_SYS_CFG -> PIPE_SYS_CRG_AHB_CLK_TOP_RST_N_REG0_CLR = 0x2;
    CPU1_SYS_CFG -> PIPE_SYS_CRG_AHB_CLK_TOP_RST_N_REG0 = 0x2;

    MODIFY_REG(PIPE_SYS_CFG -> SOFT_RST_N_REG0_CLR,
      PIPE_SYS_EMMC_CORE_CLK_SOFT_RST_N_REG0_CLR_MASK|
      PIPE_SYS_EMMC_CCLK_TX_CLK_SOFT_RST_N_REG0_CLR_MASK|
      PIPE_SYS_EMMC_TM_CLK_SOFT_RST_N_REG0_CLR_MASK|
      PIPE_SYS_EMMC_CCLK_RX_CLK_SOFT_RST_N_REG0_CLR_MASK,
      1<<PIPE_SYS_EMMC_CORE_CLK_SOFT_RST_N_REG0_CLR_POS|
      1<<PIPE_SYS_EMMC_CCLK_TX_CLK_SOFT_RST_N_REG0_CLR_POS|
      1<<PIPE_SYS_EMMC_TM_CLK_SOFT_RST_N_REG0_CLR_POS|
      1<<PIPE_SYS_EMMC_CCLK_RX_CLK_SOFT_RST_N_REG0_CLR_POS);

    MODIFY_REG(PIPE_SYS_CFG -> SOFT_RST_N_REG0,
      PIPE_SYS_EMMC_CORE_CLK_SOFT_RST_N_REG0_MASK|
      PIPE_SYS_EMMC_CCLK_TX_CLK_SOFT_RST_N_REG0_MASK|
      PIPE_SYS_EMMC_TM_CLK_SOFT_RST_N_REG0_MASK|
      PIPE_SYS_EMMC_CCLK_RX_CLK_SOFT_RST_N_REG0_MASK,
      1<<PIPE_SYS_EMMC_CORE_CLK_SOFT_RST_N_REG0_POS|
      1<<PIPE_SYS_EMMC_CCLK_TX_CLK_SOFT_RST_N_REG0_POS|
      1<<PIPE_SYS_EMMC_TM_CLK_SOFT_RST_N_REG0_POS|
      1<<PIPE_SYS_EMMC_CCLK_RX_CLK_SOFT_RST_N_REG0_POS);

    // 6. 延时6us,打开时钟门控
    DELAY_US(6);
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG1, PIPE_SYS_EMMC_AXI_CLK_CLK_CG0_5, 0x1); 
    REG_FIELD_WR(CPU1_SYS_CFG -> PIPE_SYS_CFG_REG3, PIPE_SYS_EMMC_AHB_CLK_CLK_CG0_1, 0x1);
    PIPE_SYS_CFG -> EMMC_CORE_CLK_CLK_CG0_REG = 0x1;
    PIPE_SYS_CFG -> EMMC_TM_CLK_CLK_CG0_REG = 0x1; 
    PIPE_SYS_CFG -> EMMC_CCLK_TX_CLK_CLK_CG0_REG = 0x1;
    PIPE_SYS_CFG -> EMMC_CCLK_RX_CLK_CLK_CG0_REG = 0x1;

    // 7. 延时2us, 等时钟稳定
    DELAY_US(2);

    // 8. 配置分频到400kHz
    PIPE_SYS_CFG -> EMMC_CCLK_TX_CLK_CLK_DIV0_DIV10_REG = 0x40; //25M 64分频
    PIPE_SYS_CFG -> EMMC_CCLK_RX_CLK_CLK_DEGLITCH_SW3_REG = 0x4;
    REG_FIELD_WR(CPU1_SYS_CFG -> DDR_SYS_CFG_REG2, DDR_SYS_AXI_SRAM_CLK_CLK_CG0_2, 0x1);
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_HOST_REG_VOL_STABLE, 0x1);
    /** <DONE> card_detect_n/card_write_prot config by software 软件告知emmc 卡起来了， 有实卡可选软件或者硬件pad告知*/
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_CARD_DETECT_N, 0x1);
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_CARD_WRITE_PROT, 0x1);
    // emmc clk stable
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_INT_BCLK_STABLE, 0x1);
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_INT_ACLK_STABLE, 0x1);
    REG_FIELD_WR(PIPE_SYS_CFG -> EMMC_CTL0, PIPE_SYS_EMMC_SOFT_INT_TMCLK_STABLE, 0x1);
}

void HAL_LSEMMC_MSP_DeInit(uint32_t mapbase)
{

}