...
static DWUART_HandleTypeDef *DWUART_inst_env[2];
...
void HAL_DWUART_MSP_Init(DWUART_HandleTypeDef *inst)
{
    ...
    case (uint32_t)DWUART1:
        ...
        rv_set_int_isr(DWUART1_IRQn, DWUART1_Handler);
        DWUART_inst_env[0] = inst;
        csi_vic_clear_pending_irq(DWUART1_IRQn);
        csi_vic_enable_irq(DWUART1_IRQn);
...
void HAL_DWUART_MSP_DeInit(DWUART_HandleTypeDef *inst)
{
    ...
    case (uint32_t)DWUART1:
        ...
        csi_vic_disable_irq(DWUART1_IRQn);
        break;
    case (uint32_t)DWUART2:
...
{
    rv_set_int_isr(DWUART1_IRQn, LL_DWUART1_Handler);
