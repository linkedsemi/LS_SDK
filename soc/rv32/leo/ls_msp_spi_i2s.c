...
void HAL_SPI_MSP_Init(SPI_HandleTypeDef *inst)
{
    ...
    case (uint32_t)SPI2:
        ...
        rv_set_int_isr(SPI2_IRQn, SPI2_Handler);
        spi_inst_env[0] = inst;
        csi_vic_clear_pending_irq(SPI2_IRQn);
    ...
    case (uint32_t)SPI3:
        ...
        rv_set_int_isr(SPI3_IRQn, SPI3_Handler);
        spi_inst_env[1] = inst;
        csi_vic_clear_pending_irq(SPI3_IRQn);
        csi_vic_enable_irq(SPI3_IRQn);
        break;
    }
}
...
void HAL_SPI_MSP_DeInit(SPI_HandleTypeDef *inst)
{
    ...
    case (uint32_t)SPI2:
        SYSC_PER->PD_PER_CLKG1 = SYSC_PER_CLKG_CLR_SPI2_MASK;
        csi_vic_disable_irq(SPI2_IRQn);
        break;
    case (uint32_t)SPI3:
        SYSC_PER->PD_PER_CLKG1 = SYSC_PER_CLKG_CLR_SPI3_MASK;
        csi_vic_disable_irq(SPI3_IRQn);
        break;
    }
}
...
{
    SYSC_PER->PD_PER_CLKG1 = SYSC_PER_CLKG_SET_SPI2_MASK;
    rv_set_int_isr(SPI2_IRQn, LL_SPI2_IRQHandler);
}
...
{
    SYSC_PER->PD_PER_CLKG1 = SYSC_PER_CLKG_SET_SPI3_MASK;
    rv_set_int_isr(SPI3_IRQn, LL_SPI3_IRQHandler);
}
