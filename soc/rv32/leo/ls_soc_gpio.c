...
void io_init(void)
{
    ...
    i2c_dbg_io_reinit(gpio_ien_od_value);
    for (uint8_t i = 0; i < 8; i++)
    {
        SYSC_AWO->IO[i].IEN_OD = gpio_ien_od_value[i];
        SYSC_AWO->IO[i].OE_FIR= 0;
        V33_RG->GPIO_INTR_CLR[i] = 0xffff;
        V33_RG->GPIO_INTR_CLR[i] = 0;
    }
    
    EXTI->EICR = 0xffffffff;
    EXTI->EICR = 0;
    // rv_set_int_isr(EXTI_ASYNC_IRQn,EXTI_Handler);
    // csi_vic_clear_pending_irq(EXTI_ASYNC_IRQn);
    // csi_vic_enable_irq(EXTI_ASYNC_IRQn);
    // rv_set_int_isr(EXTI1_IRQn,EXTI1_Handler);
    // csi_vic_clear_pending_irq(EXTI1_IRQn);
    // csi_vic_enable_irq(EXTI1_IRQn);
