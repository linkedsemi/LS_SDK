...
void HAL_DMA_Channel_Start_IT(DMA_Controller_HandleTypeDef *hdma,struct ch_reg *reg_cfg,void (*callback)(DMA_Controller_HandleTypeDef *,uint32_t,uint8_t,uint32_t *,uint32_t),uint32_t param)
{
...
static void int_process(DMA_Controller_HandleTypeDef *hdma,volatile uint32_t *REG_STATUS,volatile uint32_t *REG_CLR,uint32_t status_int)
{
    ...
    for(i=0;i<8;++i)
    {
        ...
        {
            *REG_CLR = 1<<i;
            hdma->channel_callback[i](hdma,hdma->param[i],i,(uint32_t *)(hdma->Instance->CH[i].LLP&~0x3),status_int);
        }
    }
}
...
void HAL_DMA_Controller_IRQHandler(DMA_Controller_HandleTypeDef *hdma)
{
    while(1)
    {
        ...
        {
            int_process(hdma,&hdma->Instance->STATUSDSTTRAN,&hdma->Instance->CLEARDSTTRAN,DMAC_DSTT_MASK);
        }
        ...
        {
            int_process(hdma,&hdma->Instance->STATUSBLOCK,&hdma->Instance->CLEARBLOCK,DMAC_SRCT_MASK);
        }
        if(status_int & DMAC_TFR_MASK)
        {
            int_process(hdma,&hdma->Instance->STATUSTFR,&hdma->Instance->CLEARTFR,DMAC_TFR_MASK);
        }
    }
