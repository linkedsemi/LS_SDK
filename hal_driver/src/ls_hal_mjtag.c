...
__attribute__((weak)) void HAL_MJTAG_TransmitReceive_CallBack(void){}
...
static void mjtag_process(MJTAG_HandleTypeDef *mjtag_h)
{
    while (mjtag_h->rx_count)
    {
        ...
        *mjtag_h->Interuupt.tdo++ = mjtag_h->MJtag->TDO;
        ...
        {
            mjtag_h->state = HAL_MJTAG_STATE_READY;
            HAL_MJTAG_TransmitReceive_CallBack();
            return;
        }
    }
