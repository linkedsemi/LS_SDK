...
static uint8_t flash_ear = 0;
static uint8_t flash_ear_save = 0;
...
static inline uint8_t hal_flash_get_ear_var()
{
    return flash_ear;
}

static inline void hal_flash_set_ear_var(uint8_t ear)
{
    flash_ear = ear;
}

uint8_t hal_flash_read_ear()
{
    uint8_t ear = 0;
    ...
    hal_flash_set_ear_var(ear);

    return ear;
}

void hal_flash_write_ear(uint8_t ear)
{
    struct flash_wr_rd_reg_param param = {
        .buf = &ear,
        .length = 1,
...
static inline void hal_flash_ear_var_save()
{
    flash_ear_save = flash_ear;
}

static inline void hal_flash_ear_var_recover()
{
    if (hal_flash_get_ear_var() != flash_ear_save) {
        hal_flash_write_ear(flash_ear_save);
    }
}

static int hal_flash_sync_ear(uint32_t offset)
{
    uint8_t wr_ear = 0;
    uint8_t rd_ear = 0;
    ...
    if (((offset & 0x00ffffff) | (hal_flash_get_ear_var() << 24)) != offset) {
        if (offset < MB(16)) {
            wr_ear = 0;
        } else if (offset >= MB(16)) {
            wr_ear = 1;
        }

        hal_flash_write_ear(wr_ear);
        rd_ear = hal_flash_read_ear();
        if (wr_ear != rd_ear) {
            return -2;
        }
    }
...
static bool is_operate_cross_border(uint32_t offset, uint16_t length)
{
    if ((offset < MB(16)) && ((offset + length) >= MB(16))) {
        return true;
    } else {
        return false;
    }
...
void hal_flash_multi_io_page_program_32bit(uint32_t offset, uint8_t *data, uint16_t length)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_multi_io_page_program(offset_low, data, length);
    hal_flash_ear_var_recover();
}

void hal_flash_dual_page_program_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_dual_page_program(offset_low, data, length);
    hal_flash_ear_var_recover();
}

void hal_flash_quad_page_program_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_quad_page_program(offset_low, data, length);
    hal_flash_ear_var_recover();
}

void hal_flash_page_program_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_page_program(offset_low, data, length);
    hal_flash_ear_var_recover();
}

void hal_flash_multi_io_read_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    if (is_operate_cross_border(offset, length)) {
        uint32_t first_length = MB(16) - offset;
        hal_flash_sync_ear(offset);
        hal_flash_multi_io_read(offset, data, first_length);

        hal_flash_sync_ear(MB(16));
        hal_flash_multi_io_read(0, data + first_length, length - first_length);
    } else {
        uint32_t offset_low = offset & 0x00ffffff;
        hal_flash_sync_ear(offset);
        hal_flash_multi_io_read(offset_low, data, length);
    }
    hal_flash_ear_var_recover();
}

void hal_flash_dual_io_read_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    if (is_operate_cross_border(offset, length)) {
        uint32_t first_length = MB(16) - offset;
        hal_flash_sync_ear(offset);
        hal_flash_dual_io_read(offset, data, first_length);

        hal_flash_sync_ear(MB(16));
        hal_flash_dual_io_read(0, data + first_length, length - first_length);
    } else {
        uint32_t offset_low = offset & 0x00ffffff;
        hal_flash_sync_ear(offset);
        hal_flash_dual_io_read(offset_low, data, length);
    }
    hal_flash_ear_var_recover();
}

void hal_flash_quad_io_read_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    if (is_operate_cross_border(offset, length)) {
        uint32_t first_length = MB(16) - offset;
        hal_flash_sync_ear(offset);
        hal_flash_quad_io_read(offset, data, first_length);

        hal_flash_sync_ear(MB(16));
        hal_flash_quad_io_read(0, data + first_length, length - first_length);
    } else {
        uint32_t offset_low = offset & 0x00ffffff;
        hal_flash_sync_ear(offset);
        hal_flash_quad_io_read(offset_low, data, length);
    }
    hal_flash_ear_var_recover();
}

void hal_flash_fast_read_32bit(uint32_t offset, uint8_t *data, uint32_t length)
{
    hal_flash_ear_var_save();
    if (is_operate_cross_border(offset, length)) {
        uint32_t first_length = MB(16) - offset;
        hal_flash_sync_ear(offset);
        hal_flash_fast_read(offset, data, first_length);

        hal_flash_sync_ear(MB(16));
        hal_flash_fast_read(0, data + first_length, length - first_length);
    } else {
        uint32_t offset_low = offset & 0x00ffffff;
        hal_flash_sync_ear(offset);
        hal_flash_fast_read(offset_low, data, length);
    }
    hal_flash_ear_var_recover();
}

void hal_flash_page_erase_32bit(uint32_t offset)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_page_erase(offset_low);
    hal_flash_ear_var_recover();
}

void hal_flash_sector_erase_32bit(uint32_t offset)
{
    hal_flash_ear_var_save();
    uint32_t offset_low = offset & 0x00ffffff;
    hal_flash_sync_ear(offset);
    hal_flash_sector_erase(offset_low);
    hal_flash_ear_var_recover();
}
