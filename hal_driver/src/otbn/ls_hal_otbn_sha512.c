...
ls_otbn_status_t HAL_OTBN_SHA512_Init()
{
    ...
    if (HAL_OTBN_IMEM_Write(0, (uint32_t *)sha512_text, SHA512_TEXT_LENTH) != LS_OTBN_OK)
        return LS_OTBN_BUSY;
    if (HAL_OTBN_DMEM_Write(SHA512_DMEM_STATE_OFFSET, (uint32_t *)state_init, SHA512_DMEM_STATE_SIZE) != LS_OTBN_OK ||
        HAL_OTBN_DMEM_Write(SHA512_DMEM_K_OFFSET, (uint32_t *)K, SHA512_DMEM_K_SIZE) != LS_OTBN_OK ||
        HAL_OTBN_DMEM_Write(SHA512_DMEM_PTR_STATE_OFFSET, &state_ptr, sizeof(uint32_t)) != LS_OTBN_OK ||
        HAL_OTBN_DMEM_Write(SHA512_DMEM_PTR_MSG_OFFSET, &msg_ptr, sizeof(uint32_t)) != LS_OTBN_OK)
        return LS_OTBN_BUSY;
    SHA512_BlockNumber_Update(0x10);
    return LS_OTBN_OK;
}

static ls_otbn_status_t msg_write(uint8_t *msg)
{
    uint8_t dword[8];
...
ls_otbn_status_t HAL_OTBN_SHA512_Update(uint8_t *msg, uint32_t length)
{
    total_cnt += length;
    if (remain_len)
    {
        ...
        if (msg_write(remain_data) != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        remain_len = 0;
        length -= wr_len;
        msg += wr_len;
...
ls_otbn_status_t HAL_OTBN_SHA512_Final(uint8_t result[SHA512_RESULT_SIZE])
{
    ...
    if (msg_write((uint8_t *)remain_data) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    SHA512_BlockNumber_Update((sha_idx - SHA512_DMEM_MSG_OFFSET) / SHA512_BLOCK_SIZE);
    if (HAL_OTBN_CMD_Write_Polling_Timeout(HAL_OTBN_CMD_EXECUTE, 20000) != LS_OTBN_OK) {
        memset(result, 0, SHA512_RESULT_SIZE);
        return LS_OTBN_TIMEOUT;
    }
...
ls_otbn_status_t HAL_OTBN_SHA512_HMAC_SetKey(uint8_t *key, uint32_t key_size)
{
    sha512_hmac_key_size = key_size;
    sha512_hmac_key = key;
    if (sha512_hmac_key_size > SHA512_BLOCK_SIZE)
    {
        if (HAL_OTBN_SHA512_Init() != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        if (HAL_OTBN_SHA512_Update(sha512_hmac_key, sha512_hmac_key_size) != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        if (HAL_OTBN_SHA512_Final(sha512_hmac_kh) != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
    ...
    if (HAL_OTBN_SHA512_Init() != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    return HAL_OTBN_SHA512_Update(sha512_hmac_kx, SHA512_BLOCK_SIZE);
}

ls_otbn_status_t HAL_OTBN_SHA512_HMAC_Update(uint8_t *msg, uint32_t msg_size)
{
    return HAL_OTBN_SHA512_Update(msg, msg_size);
}

ls_otbn_status_t HAL_OTBN_SHA512_HMAC_Final(uint8_t *out)
{
    if (HAL_OTBN_SHA512_Final(out) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    ...
    if (HAL_OTBN_SHA512_Init() != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    if (HAL_OTBN_SHA512_Update(sha512_hmac_kx, SHA512_BLOCK_SIZE) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    if (HAL_OTBN_SHA512_Update(out, SHA512_RESULT_SIZE) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    return HAL_OTBN_SHA512_Final(out);
}

ls_otbn_status_t HAL_OTBN_SHA512_HMAC(uint8_t out[SHA512_RESULT_SIZE], uint8_t *data, uint32_t data_len, uint8_t *key, uint32_t key_len)
{
    ...
    if (key_len > SHA512_BLOCK_SIZE)
    {
        if (HAL_OTBN_SHA512_Init() != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        if (HAL_OTBN_SHA512_Update(key, key_len) != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        if (HAL_OTBN_SHA512_Final(kh) != LS_OTBN_OK)
            return LS_OTBN_TIMEOUT;
        key_len = SHA512_RESULT_SIZE;
        key = kh;
    }
    ...
    if (HAL_OTBN_SHA512_Init() != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    if (HAL_OTBN_SHA512_Update(kx, SHA512_BLOCK_SIZE) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    if (HAL_OTBN_SHA512_Update(data, data_len) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
    if (HAL_OTBN_SHA512_Final(out) != LS_OTBN_OK)
        return LS_OTBN_TIMEOUT;
...
ls_otbn_status_t HAL_OTBN_SHA512_HKDF(uint8_t *salt, uint32_t salt_len,
                          ...
                          uint8_t *okm, uint32_t okm_len)
{
    ...
    if (HAL_OTBN_SHA512_HMAC(prk, ikm, ikm_len, salt, salt_len) != LS_OTBN_OK)
    ...
    {
        if (HAL_OTBN_SHA512_HMAC_SetKey(prk, SHA512_RESULT_SIZE) != LS_OTBN_OK)
            return LS_OTBN_ENGINE;
        if (T_len != 0x0) {
            if (HAL_OTBN_SHA512_HMAC_Update(T, T_len) != LS_OTBN_OK)
                return LS_OTBN_ENGINE;
        }
        if (HAL_OTBN_SHA512_HMAC_Update(info, info_len) != LS_OTBN_OK)
            return LS_OTBN_ENGINE;
        if (HAL_OTBN_SHA512_HMAC_Update(&i, 1) != LS_OTBN_OK)
            return LS_OTBN_ENGINE;
        if (HAL_OTBN_SHA512_HMAC_Final(T) != LS_OTBN_OK)
            return LS_OTBN_ENGINE;
