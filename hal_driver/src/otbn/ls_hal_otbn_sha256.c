...
#define SHA256_BLOCKSIZE (0x40)
#define SHA256_DMEM_MSG_OFFSET (0x0)
#define SHA256_DMEM_MSG_SIZE (0x40)
#define SHA256_DMEM_STATE_OFFSET (0x80)
#define SHA256_DMEM_STATE_SIZE (0x20)
#define SHA256_DMEM_MASK_OFFSET (0xa0)
#define SHA256_DMEM_MASK_SIZE (0x20)
#define SHA256_DMEM_K_OFFSET (0xc0)
...
static uint8_t remain_data[SHA256_BLOCKSIZE] = {0};
...
void HAL_OTBN_SHA256_Update(uint8_t *msg, uint32_t length)
{
    ...
    if (remain_len)
    {
        if ((length + remain_len) < SHA256_DMEM_MSG_SIZE)
        {
            memcpy(&remain_data[remain_len], msg, length);
            remain_len += length;
            return;
        }
        memcpy(&remain_data[remain_len], msg, SHA256_DMEM_MSG_SIZE - remain_len);
        msg += SHA256_DMEM_MSG_SIZE - remain_len;
        length -= SHA256_DMEM_MSG_SIZE - remain_len;
        HAL_OTBN_DMEM_Write(SHA256_DMEM_MSG_OFFSET, (uint32_t *)remain_data, SHA256_DMEM_MSG_SIZE);
        HAL_OTBN_CMD_Write_Polling(HAL_OTBN_CMD_EXECUTE);
        remain_len = 0;
    }

    for (uint32_t i = 0; i < (length / SHA256_DMEM_MSG_SIZE); i++)
    {
        HAL_OTBN_DMEM_Write(SHA256_DMEM_MSG_OFFSET, (uint32_t *)msg, SHA256_DMEM_MSG_SIZE);
        HAL_OTBN_CMD_Write_Polling(HAL_OTBN_CMD_EXECUTE);
        msg += SHA256_DMEM_MSG_SIZE;
    }
    
    if (length % SHA256_DMEM_MSG_SIZE)
    {
        memcpy(&remain_data[remain_len], msg, length % SHA256_DMEM_MSG_SIZE);
        remain_len = length % SHA256_DMEM_MSG_SIZE;
    }
}
...
void HAL_OTBN_SHA256_Final(uint8_t result[0x20])
{
    ...
    while (remain_len != 0x38) remain_data[remain_len++] = 0x0;
    for (uint8_t i = 0; i < 8; i++)
    {
        remain_data[0x3f - i] = (uint8_t)(bit_cnt >> (8 * i));
    }
    HAL_OTBN_DMEM_Write(SHA256_DMEM_MSG_OFFSET, (uint32_t *)remain_data, SHA256_BLOCKSIZE);
    HAL_OTBN_CMD_Write_Polling(HAL_OTBN_CMD_EXECUTE);
