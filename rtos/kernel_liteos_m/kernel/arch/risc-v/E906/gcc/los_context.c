...
#define INITIAL_MSTATUS                 (MSTATUS_MIE | MSTATUS_MPP | MSTATUS_MPIE | MSTATUS_FS_CLEAN)
...
{
    // LOG_I("runTask= %s",g_losTask.runTask->taskName);
    // LosTaskCB *newTask = OsGetTopTask();
    // LOG_I("newTask= %s",newTask->taskName);
    csi_vic_set_pending_irq(23);//RV_SOFT_IRQ_IDX
}

VOID HalTaskSwitch(VOID)
{
    // csi_vic_clear_pending_irq(23);
    OsSchedTaskSwitch();
    // LOG_I("runTask= %s",g_losTask.runTask->taskName);
    // LOG_I("newTask= %s",g_losTask.newTask->taskName);
